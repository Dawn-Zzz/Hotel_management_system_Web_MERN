...
const BookingForm = () => {
  ...
  const handleGuestQuantityChange = (index, i, e) => {
    const newArr = [...guestQuantity];
    newArr[index][i] = Number(e.target.value);
    setGuestQuantity(newArr);
  };
  ...
  useEffect(() => {
    ...
    if (storedRoom) {
      setRooms(JSON.parse(storedRoom).filter((room) => room.value !== "0"));
    }
  ...
  useEffect(() => {
    const newArray = rooms.map((room) => new Array(Number(room.value)).fill(1));
    setGuestQuantity(newArray);
  }, [rooms]);
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="w-1/2">
                      ...
                      ? Array.from({
                          ...
                          <div key={i} className="mt-4 mr-4">
                            ...
                            <select
                              ...
                              defaultValue="1"
                              onChange={(e) =>
                                handleGuestQuantityChange(index, i, e)
                              }
                            >
                              {Array.from({
                                length: Number(roomtype.capacity),
                              }).map((_, j) => (
          ...
          <button
            ...
            onClick={() => {
              if (guest.phoneNumber && guest.name && guest.dateOfBirth) {
                if (
                  ...
                  toast.error("Bạn chưa đủ tuổi để thực hiện hành động này");
              } else toast.error("Vui lòng điền hết thông tin cần thiết");
            }}
          >
            Book Now
          </button>
