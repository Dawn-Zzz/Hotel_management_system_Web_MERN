      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.container}>
          <View style={[styles.section, styles.leftColumn]}>
            ...
            <Text style={styles.text}>Guest Name: {data.guest.name}</Text>
            <Text style={styles.text}>
              Phone Number: {data.guest.phoneNumber}
            </Text>
            <Text style={styles.text}>
              Registration Date:{" "}
              {new Date(data.booking.createdAt).toLocaleDateString()}
            </Text>
            <Text style={styles.text}>
              Check-In Date:{" "}
              {new Date(data.booking.checkin).toLocaleDateString()}
            </Text>
            <Text style={styles.text}>
              Check-Out Date:{" "}
              {new Date(data.booking.checkout).toLocaleDateString()}
            </Text>
            <Text style={styles.text}>Staff Name: {data.staff.name}</Text>
          </View>
          <View style={[styles.section, styles.rightColumn]}>
            ...
            <Text style={styles.text}>Email: phangiadat123@gmail.com</Text>
          </View>
        </View>
        {data.booking.roomBookings && data.booking.roomBookings.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.title}>Booked Rooms</Text>
            <View style={styles.tableRow}>
              <Text style={[styles.text, styles.tableCell]}>Room Type</Text>
              <Text style={[styles.text, styles.tableCell]}>Room Number</Text>
              <Text style={[styles.text, styles.tableCell]}>Quantity</Text>
            ...
            <View style={styles.section}>
              ...
              {data.booking.serviceBookings.map((bookingId) => {
                return data.servicesBooking.data
                  .filter((serviceBooking) => serviceBooking._id === bookingId)
                  .map((serviceBooking) => {
                    return data.services.data
                      .filter(
                        (service) => service._id === serviceBooking.service
                      )
                      .map((service) => (
        ...
        <View style={styles.section}>
          <Text style={styles.text}>
            Hello {data.guest.name}, thank you for being our valuable customer.
            We hope our letter finds you in the best of health and wealth.
          </Text>
